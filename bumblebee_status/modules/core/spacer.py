...
class Module(core.module.Module):
    ...
    def __get_msg(self):
        ...
        if not len(self.__msg_queue):
            ...
            "LIVE" not in self.__msg_queue[0][1]
            and self.__msg_timer >= self.__stream_timer
        ):
            ...
            "LIVE" in self.__msg_queue[0][1]
            and self.__msg_timer >= self.__new_stream_timer
        ):
            msg = self.__msg_queue.pop(0)
            self.__msg_queue.append(msg)
            message = self.__msg_queue[0][1]
    ...
    def __remove_msg(self, stream):
        self.__msg_queue = list(filter(lambda e: e[0] != stream, self.__msg_queue))
    ...
    def __get_streams(self):
        try:
            ...
            return {"ERROR": {"game": "Connection Error", "viewers": ""}}
